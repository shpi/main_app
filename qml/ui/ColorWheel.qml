...
Rectangle {
    ...
    Slider {
        ...
        to: 1
        value: 0.5
        onMoved: {colorpicker.color = Qt.hsla((360 + (rotator.rotation % 360)) % 360 /360, 1, control.value, 1)
        ...
        background: Rectangle {
            ...
            Rectangle {
                ...
                color: "transparent"
                clip:true
                width: control.visualPosition * control.width < control.width - control.radius ?
                           (control.visualPosition * control.width)  :
                           (control.visualPosition  * control.width)
                           + ((control.visualPosition - 1 ) / (control.radius/control.width) ) * control.radius + control.radius
                ...
                width: parent.width < control.radius * 2 ? control.radius * 3 : (control.visualPosition * control.width) + control.radius
                height: parent.height
                color: "#aaffffff"
                radius: control.radius}
            }
    ...
    Text {
        ...
        text:  'R:' + (colorpicker.color.r * 255).toFixed(0).toString() +
               ' G:' + (colorpicker.color.g * 255).toFixed(0).toString() +
    ...
    MouseArea {
                ...
                onPositionChanged: {
                    ...
                    xPrev = mouse.y
                    if ( velocity > 10) {
                        if (rotator.rotation - add < 0) rotator.rotation = rotator.rotation - add
                        else rotator.rotation -= add
                  }
                  else  if ( velocity < -10) {
                        if (rotator.rotation + add > 360) rotator.rotation = rotator.rotation + add
                        else rotator.rotation += add

                  }

                    colorpicker.color = Qt.hsla((360 + (rotator.rotation % 360)) % 360 /360, 1, control.value, 1)
...
Rectangle {
    ...
    Behavior on rotation { PropertyAnimation {
       ...
       onRunningChanged:    colorpicker.color = Qt.hsla((360 + (rotator.rotation % 360)) % 360 /360, 1, control.value, 1)
...
Rectangle {
    ...
    opacity: 0 + Math.abs( (((-rotator.rotation + 3330))  - parent.rotation) % 360 - 180 )  / 40
    color: "white"
