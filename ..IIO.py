            ...
            if len(channel.attrs) > 0:
                ...
                for channel_attr in channel.attrs:
                    ...
                    if channel_attr == 'scale':
                        ...
                        try:
                            self.properties[f'{dev.name}/{channel.id}/{channel_attr}'].set = partial(IIO.write_iio, dev.id, channel.attrs['raw'].filename)
                        except Exception as e:
                            logging.error(f'Error setting value for attribute {channel_attr} on channel {channel.id} on device {dev.id}: {e}')
            ...
            for device_attr in dev.attrs:
                if device_attr == 'scale':
                    ...
                    try:
                        ...
                        logging.error(f'Error writing to attribute {device_attr} on device {dev.id}: {e}')
